    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "taxon_id": {
                    "type": "integer",
                    "description": "Species taxonomy ID (default: 9606)",
                    "default": 9606,
                },
            },
            "required": ["gene_symbol"],
        ...
        if function_name == "get_clinvar_by_gene_symbol" and isinstance(result, list):
            if len(result) > 50:
                pathogenic = sum(
                    ...
                    and "pathogenic" in v.get("clinicalSignificance", "").lower()
                )
                benign = sum(
                    1
                    for v in result
                    if isinstance(v, dict) and "benign" in v.get("clinicalSignificance", "").lower()
                )
