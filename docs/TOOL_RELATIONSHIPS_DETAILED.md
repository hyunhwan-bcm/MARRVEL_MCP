...
flowchart LR
    ...
    subgraph Input_Figures["Input Figures"]
        I_gene["Gene ID<br/>(symbol / Entrez / Ensembl)"]
        I_disease["Disease / Phenotype"]
        I_pos["Genomic Position<br/>(chr, pos, ref, alt)"]
        I_lit["Literature ID<br/>(PMID / PMCID)"]
    end
    ...
    subgraph Output_Figures_A["Output Figures"]
        O_disease["Disease Details +<br/>Phenotypes"]
        O_go["Gene Ontology<br/>Functional Profile"]
    end
    ...
    class Q queryNode
    class I_gene,I_disease,I_pos,I_lit inputNode
    class T_gene_symbol,T_gene_entrez,T_gene_ensembl,T_gene_position,T_omim_gene,T_omim_mim,T_omim_search,T_hpo_search,T_hpo_genes,T_go_gene toolNode
    class O_disease,O_go outputNode```
```
...
flowchart LR
    ...
    subgraph Variant_Conversion_And_Annotation["Variant Conversion + Annotation"]
        ...
        T_protein_change["get_protein_change_by_<br/>genomic_position"]
    ...
    subgraph Gene_Level_Variants_And_Function["Gene-Level Variants + Function"]
        ...
        T_string_pair["get_string_interactions_<br/>between_entrez_ids"]
    end

    subgraph Output_Figures_B["Output Figures"]
    ...
    I_gene --> T_clinvar_gene
    I_gene --> T_gnomad_gene
    I_gene --> T_dgv_gene
    I_gene --> T_geno2mp_gene
    I_gene --> T_diopt
    T_diopt --> T_diopt_go
    ...
    I_gene --> T_gtex
    I_gene --> T_pharos
    I_gene --> T_string_gene
    I_gene --> T_string_pair
    ...
    class Q queryNode
    class I_gene,I_hgvs,I_protein,I_rsid,I_pos inputNode
    class T_hgvs,T_protein_conv,T_rsid_conv,T_lift_38_19,T_lift_19_38,T_protein_change,T_dbnsfp,T_clinvar_var,T_gnomad_var,T_geno2mp_var,T_omim_var,T_clinvar_gene,T_gnomad_gene,T_dgv_gene,T_geno2mp_gene,T_decipher,T_diopt,T_diopt_go,T_diopt_align,T_orth_expr,T_gtex,T_pharos,T_string_gene,T_string_pair toolNode
    class O_variant,O_variant_clin,O_variant_freq,O_protein_change,O_gene_stats,O_cnv,O_ortholog,O_tissue,O_drug,O_ppi outputNode
```
